...
const AUTOGENERATED_PASSHRASE_KEY = 'autogenerated-passphrase';
const HAS_ASKED_FOR_PASSWORD_CHANGE_KEY = 'has-asked-for-password-change';
...
export class ServerSettingsService {
  ...
  #isOptionTrue(key: string, res: GetApiV1ServersettingsResponse | undefined | null) {
    if (!res) return false;
    const val = res[key];

    if ((val ?? '').trim().length == 0) return false;
    const stringval = val.trim().toLowerCase();
    return stringval === 'true' || stringval === '1' || stringval === 'yes' || stringval === 'on';
  }

  serverSettings = computed(() => this.#serverSettings() || this.#initialServerSettings());
  askForInitialPasswordConfig = computed(() => {
    const startedBy = (this.#sysinfo.systemInfo()?.StartedBy ?? '').trim().toLowerCase();
    if (startedBy === 'tray icon' || startedBy == 'trayicon') return false;

    const res = this.#serverSettings();
    const autogeneratedPassphrase = this.#isOptionTrue(AUTOGENERATED_PASSHRASE_KEY, res);
    const hasAskedForPasswordChange = this.#isOptionTrue(HAS_ASKED_FOR_PASSWORD_CHANGE_KEY, res);

    return autogeneratedPassphrase && !hasAskedForPasswordChange;
  });

  showWelcomePage = computed(() => {
    const res = this.#serverSettings();
    if (!res) return false;

    // Grandfather in previous use
    const hasAskedForPasswordChange = this.#isOptionTrue(HAS_ASKED_FOR_PASSWORD_CHANGE_KEY, res);
    const hasShownWelcomePage = this.#isOptionTrue(SHOWN_WELCOME_PAGE_KEY, res);

    return !(hasAskedForPasswordChange || hasShownWelcomePage);
  });
  ...
  isConsoleConnectionStatusHidden = computed(() => {
    return this.#isOptionTrue(HIDE_CONSOLE_CONNECTION_STATUS_KEY, this.serverSettings());
  });

  isControllerIpcEnabled = computed(() => {
    return this.#isOptionTrue(USE_OUT_OF_PROCESS_CONTROLLER_KEY, this.serverSettings());
  });
